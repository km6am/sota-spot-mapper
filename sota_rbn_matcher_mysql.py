...
class DatabaseManager:
    ...
    def find_matches(self, time_window_minutes: int = 10, freq_tolerance_hz: int = 1000):
        ...
        cursor.execute("""
            ...
            WHERE 
                ABS(TIMESTAMPDIFF(MINUTE, s.timestamp, r.timestamp)) <= %s
                AND ABS(s.frequency - r.frequency) <= %s
            ORDER BY s.timestamp DESC
        """, (time_window_minutes, freq_tolerance_hz / 1000.0))  # Convert Hz to kHz
        ...
        for match in matches:
            ...
            logger.info(f"Match found: {callsign} on {summit}, "
                       f"SOTA: {sota_freq:.3f}MHz at {sota_time} by {sota_spotter}, "
                       f"RBN: {rbn_freq:.3f}MHz at {rbn_time} by {rbn_spotter}, "
