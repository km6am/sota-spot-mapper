...
class QRZLookup:
  ...
  <ns0:Session>
    ...
    <ns0:Remark>cpu: 0.013s</ns0:Remark>
            ...
            logger.debug(f"QRZ XML response: {ET.tostring(root, encoding='utf-8').decode('utf-8')}")
            
            # Use namespace for finding Session element
            session = root.find('.//ns0:Session', namespaces={'ns0': 'http://xmldata.qrz.com'})
            if session is not None:
                logger.debug(f"QRZ Session: {ET.tostring(session, encoding='utf-8').decode('utf-8')}")
...
class DatabaseManager:
    def __init__(self, host: str = "localhost", port: int = 3306, user: str = "root", password: str = "", 
                 database: str = "spots", my_callsign: str = "", qrz_username: str = "", qrz_password: str = ""):
        self.host = host
        self.port = port
        self.user = user
        self.password = password
    ...
    def init_database(self):
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS sota_spots (
                ...
                UNIQUE KEY unique_spot (callsign, frequency, summit, timestamp, spotter)
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS rbn_spots (
                ...
                timestamp DATETIME NOT NULL,
                spotter VARCHAR(20) NOT NULL,
                mode VARCHAR(10) DEFAULT 'CW',
                is_my_callsign BOOLEAN DEFAULT FALSE,
                is_sota_matched BOOLEAN DEFAULT FALSE,
                keep_permanent BOOLEAN DEFAULT FALSE,
                UNIQUE KEY unique_spot (callsign, frequency, timestamp, spotter)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
        """)
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS matches (
                ...
                sota_id INT,
                rbn_id INT,
                sota_spotter VARCHAR(20),
                rbn_spotter VARCHAR(20),
                time_diff_seconds INT,
                freq_diff_hz INT,
                match_timestamp DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (sota_id) REFERENCES sota_spots (id) ON DELETE CASCADE,
                FOREIGN KEY (rbn_id) REFERENCES rbn_spots (id) ON DELETE CASCADE
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
        """)
        
        # Add new columns to matches table if they don't exist
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_summit_lat DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_summit_lon DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_summit_name VARCHAR(100)")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN rbn_spotter_lat DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN rbn_spotter_lon DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN rbn_spotter_name VARCHAR(100)")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_spotter_lat DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_spotter_lon DOUBLE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN sota_spotter_name VARCHAR(100)")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN location_data_enhanced BOOLEAN DEFAULT FALSE")
        except:
            pass  # Column already exists
        
        try:
            cursor.execute("ALTER TABLE matches ADD COLUMN location_enhancement_timestamp DATETIME")
        except:
            pass  # Column already exists
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS sota_locations (
                summit_ref VARCHAR(50) PRIMARY KEY,
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS rbn_locations (
                spotter VARCHAR(20) PRIMARY KEY,
                latitude DOUBLE NOT NULL,
                longitude DOUBLE NOT NULL,
                name VARCHAR(100),
                city VARCHAR(50),
    ...
    def find_matches(self, time_window_minutes: int = 10, freq_tolerance_hz: int = 1000):
        """Find matches between SOTA and RBN spots - FIXED VERSION
        
        This method now preserves existing enhanced matches instead of deleting them.
        Uses INSERT IGNORE to prevent duplicate matches while preserving location data.
        """
        conn = self.get_connection()
        ...
        try:
            ...
            # Exclude matches that already exist to avoid reprocessing
            cursor.execute("""
                SELECT 
                    s.id as sota_id, 
                    r.id as rbn_id,
                    ...
                    s.timestamp as sota_timestamp,
                    r.timestamp as rbn_timestamp,
                    s.summit,
                ...
                FROM sota_spots s
                JOIN rbn_spots r ON (
                    ABS(s.frequency - r.frequency) <= %s AND
                    ABS(TIMESTAMPDIFF(SECOND, s.timestamp, r.timestamp)) <= %s AND
                    s.callsign = r.callsign
                )
                LEFT JOIN matches m ON (m.sota_id = s.id AND m.rbn_id = r.id)
                WHERE s.timestamp >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                AND r.timestamp >= DATE_SUB(NOW(), INTERVAL 24 HOUR)
                AND m.id IS NULL
                ORDER BY s.timestamp DESC, r.timestamp DESC
            """, (freq_tolerance_hz / 1000000, time_window_minutes * 60))
            
            matches = cursor.fetchall()
            logger.info(f"Found {len(matches)} new potential matches (excluding existing)")
            
            matches_created = 0
            for match in matches:
                # Access dictionary keys instead of list indices
                sota_id = match['sota_id']
                rbn_id = match['rbn_id']
                callsign = match['callsign']
                sota_freq = match['sota_freq']
                rbn_freq = match['rbn_freq']
                sota_timestamp = match['sota_timestamp']
                rbn_timestamp = match['rbn_timestamp']
                summit = match['summit']
                sota_spotter = match['sota_spotter']
                rbn_spotter = match['rbn_spotter']
                snr = match['snr']
                
                try:
                    # Parse MySQL DATETIME format (2025-09-04 18:48:00)
                    sota_dt = datetime.strptime(str(sota_timestamp), '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
                    rbn_dt = datetime.strptime(str(rbn_timestamp), '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
                    
                    time_diff = int((rbn_dt - sota_dt).total_seconds())
                    freq_diff = int((rbn_freq - sota_freq) * 1000000)  # Convert to Hz
                    
                    # Insert using actual column names from the database schema
                    cursor.execute("""
                        INSERT IGNORE INTO matches 
                        (sota_id, rbn_id, sota_spotter, rbn_spotter, time_diff_seconds, freq_diff_hz, match_timestamp)
                        VALUES (%s, %s, %s, %s, %s, %s, %s)
                    """, (sota_id, rbn_id, sota_spotter, rbn_spotter, time_diff, freq_diff, datetime.now(timezone.utc)))
                    
                    matches_created += 1
                    
                except Exception as e:
                    logger.error(f"Error processing match {sota_id}-{rbn_id}: {e}")
                    logger.error(f"  SOTA timestamp: {sota_timestamp} (type: {type(sota_timestamp)})")
                    logger.error(f"  RBN timestamp: {rbn_timestamp} (type: {type(rbn_timestamp)})")
                    continue
    ...
    def get_my_callsign_spots(self, minutes: int = 5):
        """Get recent spots of my callsign"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        try:
            cursor.execute("""
                SELECT callsign, frequency, snr, timestamp, spotter, mode
                FROM rbn_spots 
                WHERE callsign = %s 
                AND timestamp >= DATE_SUB(NOW(), INTERVAL %s MINUTE)
                ORDER BY timestamp DESC
            """, (self.my_callsign, minutes))
            
            spots = cursor.fetchall()
            return spots
            
        except Exception as e:
            logger.error(f"Error getting my callsign spots: {e}")
            return []
        finally:
            conn.close()
    
    
    def cleanup_old_rbn_spots(self, hours: int = 24):
        """Clean up old RBN spots that are not matched to SOTA activations"""
        conn = self.get_connection()
        cursor = conn.cursor()
        
        try:
            # Delete RBN spots older than specified hours that are not matched
            cursor.execute("""
                DELETE FROM rbn_spots 
                WHERE timestamp < DATE_SUB(NOW(), INTERVAL %s HOUR)
                AND is_sota_matched = 0
            ...
            if deleted_count > 0:
                logger.info(f"Cleaned up {deleted_count} old RBN spots")
    ...
    def find_matches_for_new_spot(self, spot_id: int, callsign: str, is_sota: bool = False) -> int:
        """Find matches for a newly inserted spot"""
        conn = self.get_connection()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        
        try:
            if is_sota:
                # Find RBN spots that match this SOTA spot
                cursor.execute("""
                    SELECT 
                        %s as sota_id, 
                        r.id as rbn_id,
                    ...
                    WHERE 
                        s.id = %s
                        AND ABS(TIMESTAMPDIFF(MINUTE, s.timestamp, r.timestamp)) <= 10
                        AND ABS(s.frequency - r.frequency) <= 1.0
                        AND r.is_sota_matched = FALSE
                """, (spot_id, callsign, spot_id))
            else:
                # Find SOTA spots that match this RBN spot
                cursor.execute("""
                    SELECT 
                        s.id as sota_id, 
                        %s as rbn_id,
            ...
            for match in matches:
                ...
                try:
                    ...
                    sota_dt = datetime.fromisoformat(str(sota_timestamp).replace('Z', '+00:00'))
                # Parse MySQL DATETIME format (2025-09-04 18:48:00)
                try:
                    rbn_dt = datetime.strptime(str(rbn_timestamp), '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
                except (ValueError, TypeError):
                    # Fallback to original method for other formats
                    rbn_dt = datetime.fromisoformat(str(rbn_timestamp).replace('Z', '+00:00'))
                time_diff = int((rbn_dt - sota_dt).total_seconds())
                freq_diff = int((rbn_freq - sota_freq) * 1000000)  # Convert to Hz
                
                cursor.execute("""
                    INSERT IGNORE INTO matches 
                    (sota_id, rbn_id, sota_spotter, rbn_spotter, time_diff_seconds, freq_diff_hz)
                    VALUES (%s, %s, %s, %s, %s, %s)
                """, (sota_id, rbn_id, sota_spotter, rbn_spotter, time_diff, freq_diff))
                
                matched_rbn_ids.append(rbn_id)
                
                logger.info(f"New match found: {callsign} on {summit}, "
                           f"SOTA: {sota_freq:.3f}MHz at {sota_timestamp} by {sota_spotter}, "
                           f"RBN: {rbn_freq:.3f}MHz at {rbn_timestamp} by {rbn_spotter}, "
                           f"Δt: {time_diff}s, Δf: {freq_diff}Hz")
            
            # Mark matched RBN spots as SOTA-matched and permanent
            if matched_rbn_ids:
                placeholders = ','.join(['%s' for _ in matched_rbn_ids])
                cursor.execute(f"""
                    UPDATE rbn_spots 
                    SET is_sota_matched = TRUE, keep_permanent = TRUE
            ...
            logger.error(f"Error finding matches for new spot: {e}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            conn.rollback()
            return 0
        finally:
    ...
    def _get_cached_locations(self, conn, callsigns, summits):
        """Get cached locations from database"""
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cached = {'callsigns': {}, 'summits': {}}
        
        if callsigns:
            placeholders = ','.join(['%s'] * len(callsigns))
            cursor.execute(f"""
                SELECT spotter, latitude, longitude, name 
                FROM rbn_locations 
                WHERE spotter IN ({placeholders})
            """, list(callsigns))
            
            for row in cursor.fetchall():
                cached['callsigns'][row['spotter']] = Location(
                    row['latitude'], row['longitude'], row['name'] or row['spotter']
                )
        
        if summits:
            placeholders = ','.join(['%s'] * len(summits))
            cursor.execute(f"""
                SELECT summit_ref, latitude, longitude, name 
                FROM sota_locations 
                WHERE summit_ref IN ({placeholders})
            """, list(summits))
            
            for row in cursor.fetchall():
                cached['summits'][row['summit_ref']] = Location(
                    row['latitude'], row['longitude'], row['name'] or row['summit_ref']
                )
        
        cursor.close()
        return cached
    
    def _batch_fetch_callsign_locations(self, callsigns):
        """Batch fetch callsign locations from QRZ.com"""
        locations = {}
        batch_size = 10  # QRZ.com rate limiting
        
        for i in range(0, len(callsigns), batch_size):
            batch = callsigns[i:i + batch_size]
            logger.info(f"Fetching QRZ data for batch {i//batch_size + 1}: {batch}")
            
            for callsign in batch:
                try:
                    location = self.get_rbn_location(callsign)
                    if location:
                        locations[callsign] = location
                    time.sleep(0.1)  # Rate limiting
                except Exception as e:
                    logger.warning(f"Failed to fetch location for {callsign}: {e}")
            
            # Longer delay between batches
            if i + batch_size < len(callsigns):
                time.sleep(1)
        
        return locations
    
    def _batch_fetch_sota_locations(self, summits):
        """Batch fetch SOTA summit locations"""
        locations = {}
        
        for summit in summits:
            try:
                location = self.get_sota_location(summit)
                if location:
                    locations[summit] = location
                time.sleep(0.1)  # Rate limiting
            except Exception as e:
                logger.warning(f"Failed to fetch SOTA location for {summit}: {e}")
        
        return locations
    
    def _cache_locations(self, conn, locations):
        """Cache callsign locations in database"""
        if not locations:
            return
        
        cursor = conn.cursor()
        try:
            data = [(callsign, loc.latitude, loc.longitude, loc.name) 
                   for callsign, loc in locations.items()]
            
            cursor.executemany("""
                INSERT INTO rbn_locations (spotter, latitude, longitude, name)
                VALUES (%s, %s, %s, %s)
    ...
    def _cache_sota_locations(self, conn, locations):
        """Cache SOTA summit locations in database"""
        if not locations:
            return
        
        cursor = conn.cursor()
        try:
            data = [(summit, loc.latitude, loc.longitude, loc.name) 
                   for summit, loc in locations.items()]
            
            cursor.executemany("""
                INSERT INTO sota_locations (summit_ref, latitude, longitude, name)
                VALUES (%s, %s, %s, %s)
                ON DUPLICATE KEY UPDATE
                latitude = VALUES(latitude),
                longitude = VALUES(longitude),
    ...
    def enhance_matches_with_location_data(self):
        """Batch enhance matches with location data using MySQL bulk operations"""
        conn = self.get_connection()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        ...
        try:
            ...
            while True:
                # Get batch of unenhanced matches
                cursor.execute("""
                    SELECT 
                        m.id as match_id,
                        s.callsign,
                    ...
                    WHERE m.match_timestamp > DATE_SUB(NOW(), INTERVAL 2 HOUR)
                    AND (m.location_data_enhanced IS NULL OR m.location_data_enhanced = 0)
                ...
                if not matches:
                    if total_enhanced == 0:
                        logger.info("No unenhanced matches found for location enhancement")
                    else:
                        logger.info(f"Completed enhancement of all unenhanced matches. Total enhanced: {total_enhanced}")
                    break
                
                logger.info(f"Processing batch of {len(matches)} unenhanced matches")
                
                # Collect unique callsigns and summits for batch lookup
                unique_callsigns = set()
                unique_summits = set()
                
                for match in matches:
                    unique_callsigns.add(match['callsign'])
                    unique_callsigns.add(match['sota_spotter'])
                    unique_callsigns.add(match['rbn_spotter'])
                    unique_summits.add(match['summit'])
                
                # Batch fetch all locations
                logger.info(f"Fetching locations for {len(unique_callsigns)} callsigns and {len(unique_summits)} summits")
                
                # Get cached locations from database first
                cached_locations = self._get_cached_locations(conn, unique_callsigns, unique_summits)
                
                # Determine what needs to be fetched from APIs
                callsigns_to_fetch = [c for c in unique_callsigns if c not in cached_locations['callsigns']]
                summits_to_fetch = [s for s in unique_summits if s not in cached_locations['summits']]
                
                # Batch fetch from APIs
                if callsigns_to_fetch:
                    logger.info(f"Fetching {len(callsigns_to_fetch)} callsigns from QRZ.com")
                    api_locations = self._batch_fetch_callsign_locations(callsigns_to_fetch)
                    self._cache_locations(conn, api_locations)
                    cached_locations['callsigns'].update(api_locations)
                
                if summits_to_fetch:
                    logger.info(f"Fetching {len(summits_to_fetch)} summits from SOTA API")
                    sota_locations = self._batch_fetch_sota_locations(summits_to_fetch)
                    self._cache_sota_locations(conn, sota_locations)
                    cached_locations['summits'].update(sota_locations)
                ...
                for match in matches:
                    ...
                    sota_location = cached_locations['summits'].get(summit)
                    rbn_location = cached_locations['callsigns'].get(rbn_spotter)
                    sota_spotter_location = cached_locations['callsigns'].get(sota_spotter)
                    
                    if sota_location or rbn_location or sota_spotter_location:
                        update_data.append((
                            sota_location.latitude if sota_location else None,
                            sota_location.longitude if sota_location else None,
                            sota_location.name if sota_location else None,
                            rbn_location.latitude if rbn_location else None,
                            rbn_location.longitude if rbn_location else None,
                            rbn_location.name if rbn_location else None,
                            sota_spotter_location.latitude if sota_spotter_location else None,
                            sota_spotter_location.longitude if sota_spotter_location else None,
                            sota_spotter_location.name if sota_spotter_location else None,
                            match_id
                        ))
                
                # Batch update matches
                if update_data:
                    logger.info(f"Batch updating {len(update_data)} matches with location data")
                    cursor.executemany("""
                        UPDATE matches 
                        SET 
                            sota_summit_lat = %s,
                            ...
                            sota_spotter_lon = %s,
                            sota_spotter_name = %s,
                            location_data_enhanced = TRUE,
                            location_enhancement_timestamp = NOW()
                        WHERE id = %s
                    """, update_data)
                    
                    conn.commit()
                    total_enhanced += len(update_data)
                    logger.info(f"Successfully enhanced {len(update_data)} matches")
                
                # Mark remaining matches as processed (even if no location data found)
                remaining_match_ids = [m['match_id'] for m in matches if m['match_id'] not in [d[9] for d in update_data]]
                if remaining_match_ids:
                    cursor.execute("""
                        UPDATE matches 
                        SET location_data_enhanced = TRUE, location_enhancement_timestamp = NOW()
                        WHERE id IN ({})
                    """.format(','.join(['%s'] * len(remaining_match_ids))), remaining_match_ids)
                    conn.commit()
                    logger.info(f"Marked {len(remaining_match_ids)} matches as processed (no location data available)")
                
                # If we got fewer matches than the batch size, we've processed all available matches
            ...
            logger.error(f"Error enhancing matches with location data: {e}")
            logger.error(f"Traceback: {traceback.format_exc()}")
            conn.rollback()
            return 0
        finally:
    ...
    def get_sota_location(self, summit_ref: str) -> Optional[Location]:
        ...
        try:
            ...
            if result:
                return Location(result['latitude'], result['longitude'], result['name'] or summit_ref)
            ...
            logger.error(f"Error getting SOTA location for {summit_ref}: {e}")
        finally:
            if conn.open:
                conn.close()
    ...
    def get_rbn_location(self, spotter: str) -> Optional[Location]:
        ...
        try:
            ...
            logger.error(f"Error getting RBN location for {spotter}: {e}")
            return None
        finally:
            if conn.open:
                conn.close()
    ...
    def get_propagation_paths(self, minutes: int = 1440) -> List[PropagationPath]:
        ...
        cursor.execute("""
            ...
            JOIN sota_spots s ON m.sota_id = s.id
            JOIN rbn_spots r ON m.rbn_id = r.id
            WHERE m.match_timestamp > DATE_SUB(NOW(), INTERVAL %s MINUTE)
            ORDER BY m.match_timestamp DESC
        """, (minutes,))
...
<body>
    ...
    <div class="info-panel">
        ...
        <div id="stats">
            ...
            <div id="timestamp-info">
                <small>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}</small><br>
                <small id="age-info">Age: <span id="age-display">calculating...</span></small>
            </div>
    ...
    <script>
        ...
        // Page generation time and age calculation
        var pageGeneratedTime = new Date('{datetime.now().strftime('%Y-%m-%dT%H:%M:%S')}Z');
        
        function updateAge() {{
            var now = new Date();
            var ageMs = now - pageGeneratedTime;
            var ageMinutes = Math.floor(ageMs / (1000 * 60));
        ...
        // Update age immediately and then every minute
        updateAge();
...
class SOTAClusterClient:
    ...
    def parse_sota_spot(self, line: str) -> Optional[SOTASpot]:
        ...
        try:
            ...
            # Log SOTASpot if debug flag is True
            if self.debug:
                logger.debug(f"SOTASpot: {spot}")
            ...
            if spot:
                ...
                if self.db_manager.is_sota_spot_recent(spot):
                    ...
                    if spot_id:
                        ...
                        # Immediately try to find matches for this new spot
                        matches_found = self.db_manager.find_matches_for_new_spot(spot_id, spot.callsign, is_sota=True)
                        if matches_found > 0:
                            logger.info(f"Found {matches_found} new matches for SOTA spot {spot_id}")
                            
                            # Retrieve QRZ and SOTL.AS information for new matches
                            self.db_manager.enhance_matches_with_location_data()
                else:
...
class RBNClusterClient:
    ...
    def parse_rbn_spot(self, line: str) -> Optional[RBNSpot]:
        ...
        try:
            ...
            # Log RBNSpot if debug flag is True
            if self.debug:
                logger.debug(f"RBNSpot: {spot}")
        ...
        while self.running:
            ...
            if spot:
                ...
                if spot_id:
                    ...
                    # Immediately try to find matches for this new spot
                    matches_found = self.db_manager.find_matches_for_new_spot(spot_id, spot.callsign)
                    if matches_found > 0:
                        logger.info(f"Found {matches_found} new matches for RBN spot {spot_id}")
...
class SpotMatcher:
    def __init__(self, callsign: str = "N0CALL", my_callsign: str = "", qrz_username: str = "", qrz_password: str = "", 
                 mysql_host: str = "localhost", mysql_port: int = 3306, mysql_user: str = "root", 
                 mysql_password: str = "", mysql_database: str = "spots", debug: bool = False):
        self.callsign = callsign
        self.my_callsign = my_callsign.upper() if my_callsign else callsign.upper()
        self.debug = debug
        self.db_manager = DatabaseManager(host=mysql_host, port=mysql_port, user=mysql_user, 
    ...
    def _match_loop(self):
        ...
        while self.running:
            ...
            try:
                ...
                # Enhance location data every 5 minutes
                enhancement_counter += 1
                if enhancement_counter >= 5:
                    enhancement_counter = 0
                    enhanced = self.db_manager.enhance_matches_with_location_data()
                    if enhanced > 0:
                        logger.info(f"Enhanced {enhanced} matches with location data")
    ...
    def get_recent_matches(self, hours: int = 24) -> List[Tuple]:
        ...
        cursor.execute("""
            ...
            JOIN sota_spots s ON m.sota_id = s.id
            JOIN rbn_spots r ON m.rbn_id = r.id
            WHERE m.match_timestamp > DATE_SUB(NOW(), INTERVAL %s HOUR)
            ORDER BY m.match_timestamp DESC
        """, (hours,))
    ...
    def get_propagation_stats(self, hours: int = 24) -> Dict:
        """Get propagation statistics"""
        paths = self.db_manager.get_propagation_paths(hours)
...
def main():
    ...
    my_callsign = config.get("callsigns", {}).get("my_callsign", "N0CALL")
    cluster_callsign = config.get("callsigns", {}).get("cluster_callsign", "N0CALL")
    
    # QRZ credentials (optional but recommended for accurate locations)
    qrz_username = config.get("credentials", {}).get("qrz", {}).get("username", "N0CALL")
    qrz_password = config.get("credentials", {}).get("qrz", {}).get("password", "")
    
    # MySQL database credentials
    mysql_host = config.get("credentials", {}).get("mysql", {}).get("host", "localhost")
    mysql_port = config.get("credentials", {}).get("mysql", {}).get("port", 3306)
    mysql_user = config.get("credentials", {}).get("mysql", {}).get("user", "root")
    mysql_password = config.get("credentials", {}).get("mysql", {}).get("password", "")
    mysql_database = config.get("credentials", {}).get("mysql", {}).get("database", "spots")
    ...
    try:
        ...
        # Run for a while
        while 1:
            time.sleep(refresh_interval)  # Configurable refresh interval

            # Show propagation statistics
            stats = matcher.get_propagation_stats(history_window)  # Configurable window
            logger.info(f"=== Propagation Statistics (Last {history_window} Hour(s)) ===")
            logger.info(f"Total propagation paths: {stats.get('total_paths', 0)}")
            if stats.get('total_paths', 0) > 0:
                logger.info(f"Unique SOTA summits: {stats.get('unique_summits', 0)}")
                logger.info(f"Unique RBN spotters: {stats.get('unique_spotters', 0)}")
                logger.info(f"Average distance: {stats.get('avg_distance_km', 0):.1f} km")
                logger.info(f"Distance range: {stats.get('min_distance_km', 0):.1f} - {stats.get('max_distance_km', 0):.1f} km")
                logger.info(f"Average SNR: {stats.get('avg_snr_db', 0):.1f} dB")

                logger.info("Frequency band distribution:")
                for band, count in stats.get('frequency_bands', {}).items():
                    logger.info(f"  {band}: {count} paths")

                # Generate interactive map
                map_file = matcher.generate_map(map_window)  # Configurable window
                if map_file:
                    logger.info(f"Interactive propagation map saved as: {map_file}")

            # Show my recent spots
            my_spots = matcher.get_my_recent_spots(recent_spots_window)  # Configurable window
            if my_spots:
                logger.info(f"My callsign ({my_callsign}) heard in the last {recent_spots_window} minutes:")
                for spot in my_spots[:10]:  # Show first 10
                    callsign, freq, snr, timestamp, spotter, mode = spot
                    logger.info(f"  {callsign} {float(freq):.1f}kHz {snr}dB {mode} by {spotter} at {timestamp}")
            else:
                logger.info(f"No spots of {my_callsign} in the last {recent_spots_window} minutes")

            # Show recent SOTA matches
            matches = matcher.get_recent_matches(history_window)  # Configurable window
            if matches:
                logger.info(f"Found {len(matches)} SOTA/RBN matches in the last {history_window} hour(s):")
                for match in matches[:10]:  # Show first 10
                    try:
                        ...
                        if isinstance(match, dict):
                            ...
                            freq_diff = match['freq_diff_hz']
                            snr = match['snr']
                            sota_spotter = match['sota_spotter']
                            rbn_spotter = match['rbn_spotter']
                        else:
                            callsign, summit, sota_freq, rbn_freq, sota_timestamp, rbn_timestamp, time_diff, freq_diff, snr, sota_spotter, rbn_spotter = match
                        
                        logger.info(f"  {callsign} on {summit}: SOTA {float(sota_freq):.3f}MHz (by {sota_spotter}) -> RBN {float(rbn_freq):.1f}kHz (by {rbn_spotter}) "
                              f"({time_diff}s, {freq_diff}Hz, {snr}dB)")
                    except (ValueError, TypeError, KeyError) as e:
                        logger.error(f"Error formatting match data: {e}")
                        logger.error(f"Match data: {match}")
                        logger.error(f"Match type: {type(match)}")
            else:
                logger.info(f"No SOTA/RBN matches in the last {history_window} hour(s)")
